...
**唯一的例外出口**：单件可声明 `budget: { meshes, verts }` 覆盖上限（契约测试认它）。
规矩（2026-09-11 定）：
- 只在"这一件必须长期扛近景表演"时用（例：老虎机——休息房主角、要怼脸看、还要长出
  粉碎口/计数器这类新入口），且必须在文件头注释里写明**为什么值得**与**回本计划**；
- 先穷尽低成本的账：**静态子件按材质族合并**（`mergeStatic` 对件内局部变换同样适用）、
  **重复小件走 InstancedMesh + instanceColor**（一圈彩灯/按键/铆钉这类：十几件 → 1 件，
  逐实例颜色照样能驱动）；这两招能把网格数降一个量级而不掉细节；
- 顶点上限确实要过时再写进去，并在注释里给出实测数字（`?` 验收页会打印 mesh/verts）。
