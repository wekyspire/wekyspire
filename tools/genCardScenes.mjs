...
const SCENES = [
  ...
  { id: 'agileCombo', prompt: `two blurred afterimage fists landing a snappy one-two in mid-air, echo trails behind them, the rest of the frame empty dark, minimal and fast` },
  ...
  { id: 'crashLanding', prompt: `ground-level view looking up: a small stretched black silhouette dropping out of the dark sky elbow-first, a shockwave ring already cracking the earth beneath him, scale contrast` },
  ...
  { id: 'doubleStrike', prompt: `twin afterimage fists striking the same point, two impact bursts side by side, tight close crop, nothing else in frame` },
  ...
  // —— 通用灰卡家族（骑士的日常动作） ——
  { id: 'purify', prompt: `a huge clear water drop washing over a steel-gray gauntlet in close-up, a dark stain dissolving off the plates, brilliant purity` },
  ...
  { id: 'drawQi', prompt: `thin streams of pale breath-light converging from every edge of the frame toward one small dark silhouette at the center, inhaling motion` },
  ...
  { id: 'expandChant', prompt: `a row of small colored chant lights floating across the dark frame — green yellow blue — above two upturned steel-gray palms in close-up` },
...
const T2I_TIERS = {
  'agileCombo-2': 'a diagonal row of six separate glowing pale fists marching across the dark frame, motion blur trails behind each fist, quick rhythmic combo energy',
  ...
  'relief-3': 'a colossal roaring white steam explosion erupting sideways and filling the entire frame edge to edge, a tiny round pressure valve barely visible at the bottom corner',
...
const ESCALATION_VOID = {
  ...
  wildFlurry: { // 乱拳(C)→雨拳(B)→千手(A)→万手(S)：万手归一、一片空茫
    2: ', the same rain of fists, denser and faster, more blurred fists',
    3: ', the same rain, the fists dissolving into gray mist, the color draining away',
...
const ESCALATION_CUSTOM = {
  ...
  bloodFist: { // 血拳(B)→血拳(A)：血更盛
    3: ', the same blood sea, the blood far more vivid and brighter, glowing with stolen life, the arm plunged deeper',
  },
  ...
  annihilatingEdge: { // 斩灭(A)→斩灭(S)：刃口噬光（首轮「drinking the light」语义太玄，构图漂了——改死锚 pose）
    4: ', the exact same dark blade filling the frame, its edge turning void-black with a thin annihilating pale rim, the background darker',
  },
  ...
  cleave: { // 横劈(C)→强力劈(B)→裂空劈(A)：裂空 = 劈开空气
    2: ', the same flat horizontal arc, wider and brighter, the air splitting behind the edge',
  ...
  feint: { // 假动作(C→B→A)：影分身逐级凝实
    2: ', the same two silhouettes, the ghost double more solid and convincing, harder to tell apart',
  ...
  flyingDagger: { // 飞刀(C)→强力飞刀(B)→绝灭飞刀(A)：刀尾光轨
    2: ', the same throwing knife, a sharper longer speed trail, faster and heavier',
    3: ', the same knife, a long bright screaming trail, the tip glowing, annihilating momentum',
  },
  handCleave: { // 花刀(C→B)→蔽目花刀(A)：蔽目 = 耀目刀幕
    2: ', the same circle of blade-light, a brighter wider arc trail riding the circle',
    3: ', the same circle, a dazzling blinding fan of arc light veiling the frame',
  },
  ...
  woodSting: { // 飞刺：刺雨逐密
    1: ', the same thorny stinger, a second smaller one trailing it',
    2: ', the same thorny stingers, a wide volley of identical stingers fanning out',
  ...
  windBlade: { // 风刃：刃逐大
    1: ', the same cyan crescent, larger and brighter',
    2: ', the same crescent, huge and bright with gust rings trailing',
    3: ', the same crescent, a massive tearing blade-storm, double crescents, the air ripped open',
  },
  psiShield: { // 灵能屏障：两层屏障显形（两轮「更大更亮」都读不出——屏障已大，加新层）
    2: ', the same pale light blue hexagon sphere, far brighter and more vivid, blazing light-blue facets filling the frame, the tiny silhouette inside completely empty-handed with arms at its sides, no shield, no staff, no objects anywhere',
  },
  relief: { // 泄压：蒸汽逐猛
    1: ', the same valve, a wider steam blast',
    2: ', the same valve fully wrenched open, a huge roaring white steam blast flooding half the frame',
    3: ', the same pose, a colossal white steam explosion filling the entire frame edge to edge, nothing else visible',
  },
  silence: { // 灭烛：烟缕逐多、暗逐深
  ...
  heavyStomp: { // 扫堂腿→旋风腿：尘浪逐级成暴
    2: ', the same sweeping boots, faster, a wider dust wave, deeper motion blur',
    3: ', the same boots, a roaring dust wave crashing across the frame, debris blasted up',
    4: ', the same boots spinning into a full cyclone of dust and debris circling the frame',
  },
  duckHead: { // 抱头→格挡→完美格挡：臂甲墙上火花逐级成暴、终阶吸能生辉
    2: ', the same wall of crossed vambraces, harder sparks bursting on the plates',
  ...
  forgingBlade: { // 锻刀术：锻花逐级成瀑
    2: ', the same forging strike, brighter sparks bursting off the hot blade',
    3: ', the same strike, a huge spray of forge sparks flooding the frame, the blade blazing white-hot',
  },
  fireworks: { // 烟花：礼花逐级满开
    2: ', the same burst, more colorful shells blooming across the sky',
    3: ', the same scene, a whole festival of fireworks blooming everywhere, color filling the frame',
  },
  residualHeatPlus: { // 余热+：余烬复燃
    3: ', the same gauntlet, hotter: the embers rekindling into small flames, orange glow rising again',
  },
  hotHands: { // 烫手：指火逐级成焰
  ...
  spark: { // 火花：花爆逐级满框（人物锁定：宽檐帽+护目镜，flash 取读自基图）
    2: ', the same figure with the same wide-brim hat and goggles, a far bigger burst with twice as many bright sparks flying',
    3: ', the same figure with the same wide-brim hat and goggles, a storm of blazing sparks flooding the entire frame',
  },
  redHotBlade: { // 红热刃：刃温逐级至白热
    2: ', the same blade, hotter: brighter red glow, heavier heat shimmer',
    3: ', the same blade near-molten, a white-hot core, embers streaming off the edge',
  },
  ...
  prePrepared: {
    ...
    3: ', the same armored figure with the same colorful umbrella, dramatic rim light, the umbrella held like a sword',
  },
  holdOut: {
