...
// 转阶段「燃躯解放」（转段首拍空转 = 本剧本，约 1.4s 阻塞节拍）：
...
//     ⑤ 环绕火球狂暴：部件寻址 unit.parts.get('orbs')，轨道半径/转速/尺寸直推
...
async function pyroP2({ ctx, args, cast, particles, shake, vignette, camera, onStageDispose, runScript, unitById }) {
  ...
  camera?.pushOverride?.('pyroP2', {
    poses: { pyroP2: { position: { x: 0, y: 7, z: 130 }, lookAt: { x: -4, y: -8, z: -40 } } },
    onEnter: (dir) => { dir.flyTo('pyroP2', { durationMs: 1800 }); }, // fire-and-forget：飞行与爆发并行
  });
  ...
  (runScript ?? ((body) => { ctx.spawn(body); }))(async (c) => {
    ...
    for (const { handle } of cast.query('prop:brazierFire')) {
      ...
      jobs.push(c.tweenRaw(em, { rate: 14 }, { durationMs: 1800 }));
    }
    for (const [ex, ez] of [[-10, -20], [8, -35]]) { // 战线中轴上空再飘两团
      ...
      if (!em) continue;
      onStageDispose?.(() => em.stop());
      jobs.push(c.tweenRaw(em, { rate: 10 }, { durationMs: 2200 }));
    }
    // ⑤ 环绕火球狂暴（多部件寻址：轨道参数公开可写，直推即演出）
