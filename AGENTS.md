...
- **`run/`** — run 层状态机：`runFlow.js`（createRun/enterBattle/finishBattle/advanceFloor…）、`runDriver.js`（headless 整局 SDK，自动应答结算期输入、自动开包与种子包）、rewards / ascension / prep、rooms/（camp、event、slotMachine、training）。**Boss 通关奖励三件套**：删卡机会 + HP 回满 + **遗物掉落**（`BOSS_RELIC_RARITY` 按章爬坡：章1 C/B、章2 B、章3 B/A、章4 A；走 draft.js 统一抽选 SDK，特写由 Shell 拥有集差分自动兜——2026-09-13 实装：设计稿早列了这个来源但一直没人写，导致遗物正常局一局只见 0-2 件）。
  - **奖励卡包制**（2026-09）：战后先选卡包再包内三选一；**基础包（原体修包，2026-09-14 更名）恒开（门禁看隐藏的 `player.bodyLevel`）**，灵脉包需 `leino[维度] ≥1`（门禁看该维度等级：0 级 D/C、1 级 B、2 级 A——专精昂贵是设计意图）；木/空无内容时自动隐藏；训练房抓牌走「已解锁卡包并集」。**基础包通用卡注入浓度大幅提升（2026-09-14 用户定，体修成型第三刀）**：主注入 90%+保底 2，命中后 50% 再换第二张（期望每包 ≈1.35 张通用卡）——通用卡是强力单卡但不能成体系（C 位）：灵脉玩家开基础包收益升，体修玩家的体修候选被稀释，前期成型压力增加。
  ...
  - **跳过进阶**：不选灵脉，改记 1 点隐藏体修等级（`player.bodyLevel`，体修卡包门禁）+ **生命上限 +3**（第 7 轮裁决的跳过补偿，走 `gainMaxHp`）+ **删卡机会 +1**（体修吃牌组纯净度，删卡即这条路线的成型资源），**不回血、不提魏启**（2026-09-14 用户收紧：此前跳过四项全给——+3HP/回10/+1魏启/删卡——六路试玩里全跳过路线横扫 44/38/32 三席，「难成型、成型后极强」定位倒挂；血量/魏启类通用资源只有点火灵脉才给），不触发种子包——故事模式暗线（体修大成）。
...
- **设计稿术语口径（用户定 2026-09-15，防臆测乌龙）**：实现设计文档里的卡面效果语言前，先按既定主语约定读稿——**裸写「效果X N」（如「燃烧3」「格挡2」）= 赋予自己**效果 X N 层；**「赋予效果X / 目标获得X」= 赋予卡牌目标**（通常是敌人）。**术语不理解务必查权威文档**：效果规则查 `battle_gameplay/skills/EFFECTS.md`、机制词查 `core/skills/namedTerms.js`（NAMED）与对应体系设计稿——**禁止凭语感臆测**；NAMED 已定义的语义与写法（含尾缀数字参数如 `衰败2`）必须照办，不得自创同义写法。**查不到定义的术语 = 不实现**，向用户上报待定义（臆测实现是乌龙重灾区）。
- 卡面描述双轨：`describe`（应用前，无战斗上下文，纯文本）/ `battleDescribe`（结算中，数字按实时局面）。
- **新卡数值意识（用户定 2026-09-13，设计铁律）**：设计新卡必须先做数值对标——与同阶基础卡、其他体系的同类功能卡逐项比（盾对盾、伤对伤、抽对抽），确认落在既有包络内再写定义，**不许凭感觉给数**（火墙初版 12 盾 +6、B 阶 0 费无冷却，横向对比 D 阶盾/体修盾穿包络，被用户打回=反面教材）。**彻底 0 开销卡（0AP+0 魏启+无冷却）必须谨慎**——免费即每回合白嫖，默认冷却 1 起步。条件加成卡的拆分口径：**基础值对标同阶白板、加成值才是体系溢价**，无条件部分不许比同阶白板强（火墙定稿：基础 7、燃烧加成 +7/+11/+11）。
