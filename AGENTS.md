...
- **后处理层 `stage/post/`（2026-09-26 结构大更新 Phase 0）**：全屏 pass 唯一事实源 `passes.js`（VERT/FRAG_BRIGHT/FRAG_BLUR/FRAG_FINAL/`GLSL_TONE_LIB`/`TONE_MODES`/`applyToneMapping` + `makeFullScreenPass` 三帮手）、`bloomChain.js`（bright→半分辨率 H/V blur 三段）、`uiComposer.js`（**uiScene 后处理链**：卡牌/UI → HalfFloat RT(MSAA×4) → bloom → 终段 tone map，premultiplied 盖回屏幕）。体积月光 `scenes/volumetricMoon.js` 与 uiComposer 共享同一套 pass 实现（阈值 1.45 等约定只有一份）。**RT 合成两条铁律**：① 终段 premultiplied（RT 里 rgb 本就是预乘色、alpha 是真覆盖率，与直渲逐像素等价）；② **uiScene 加法发光件一律 `additiveLight()`**（rgb 加算照旧、alpha 不占地）——普通 AdditiveBlending 会让光斑在 RT 里「占地」挡世界，新发光 UI 不改混合就会踩。HDR 创作约定：内容色 ≤1，发光件乘算推过 1.45 阈值才进 bloom（咏唱流光是首个受益件）。开关：`settings.fxPost`（菜单「辉光特效」，默认开）→ `stageManager.setUiPostProcessing`，关掉回旧直渲；`?uipost=0` 强制关。
- **单位特效固定层级（2026-09-26 结构大更新；用户定 4 层 L0 本体/L1 贴体/L2 笼罩/L3 表意，四层已全落）**：统一宿主 = `fx/unitFxLayer.js` 的 **UnitFxLayer**（UnitObject 构造时自建，对位 CardFxLayer）：持有 L0 补丁记录 + L1–L3 分组（挂 **billboard 子组**——贴体铁律：挂 unit 根的件不随立绘转身）+ overlay 槽位池（key→句柄，幂等 `overlay()`/`setLevel()`/`clear()`）+ 常驻推进 uTime（多 uniform 共用一钟，不许单 aura 占钟）+ 统一 dispose。**L0 本体层** = `fx/unitBodyFx.js`，onBeforeCompile 挂单位 `_body` 材质（**每单位独立实例，无族单例复位负担**），单 program 多 uniform（uBurn/uPoison，片元内固定顺序合成 = 上层天然读下层输出），着色链抽成 `GLSL_BODY_FX` 共享件；本体亮度**压 bloom 阈下**（发光是上层的活，防糊白）；BattleStage 入场 `compileAsync` 预热（编译卡顿 715ms 教训）。**L1 贴体层** sprite 叠层（z 0.50~0.70）：`bodyFlames.js` 叠火（HDR 乘算过 1.45 阈真发光）/`vaporPlumes.js` 毒雾（哑光阈下）。**L2 笼罩层**（z 0.80，需下层状态的壳罩件）：`stasisShell.js` 凝滞壳 = **同源重算参考实现**——采样同一张立绘 map + include `GLSL_BODY_FX` + **共享本体 uniform 实例**，零 RT 复现下层燃烧/中毒着色（真邻域采样才懒建 RT）；挂 `_standee` 随姿态冻结，setArt 换几何/map 后到都靠 tick 自愈重绑。**L3 表意层**（z 0.95）：`stunStars.js` 眩晕星。状态入口唯一 = AuraHost + `recipes.js` 的 `UNIT_AURA_THEMES`（**层级/槽位/建件器/levelOf 声明式主题表**，aura def 是纯「槽位导演」——enter/update/exit 只推 level 标量不管建件；diff 驱动铁律不动；**存续 aura 强度追层数走 `def.update`**）。aura 补间两条纪律：`overwrite:'auto'` + `onInterrupt` 同落地。**tick 件容错纪律**：`UnitObject.update` 逐件 try/catch + 就地摘除病灶件（vaporPlumes 事故——异常逃出 update 打断 rAF 主循环，画面冻结但 gsap 独立补间继续推 uniform，数据断言全绿视觉已死；验收必须查 tick 活性如 uTime 推进，不能只信 uniform 读数）。
...
- **对象**：`objects/`（CardObject、UnitObject、ZonePileObject、TargetingArrowObject、**ItemShowcaseObject** 公共获得演出（`Stage.showcaseItem`；`skippable:true` 双出口）、**SpeechBubbleObject/BubbleLayer** 头顶泡泡（放 uiScene 恒定屏幕尺寸）、PanelObject（锚定/模态/dock 形态）、ButtonObject、TextBlockObject、ScrollPickerObject 全屏选择骨架、CardScrollPicker/RelicScrollPicker、SlotRollObject 等）。**卡牌变换演出走 `fx/cardTransform.js`**（模式注册表 `CARD_TRANSFORM_MODES`，默认 charReveal = 焦化→燃烧尾迹→白光新脸的双前锋 wipe；**双脸难题 = 预烘焙新脸 + 叠层双纹理同屏渲染，落幕一刻才 `applyBakedFace`**——调用点不许先 setCard；在途演出挂 `view._transformCancel`，dispose/startBurn/二次变换掐死）。
