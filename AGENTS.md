...
- **单位特效固定层级（2026-09-26 结构大更新；用户定 4 层 L0 本体/L1 贴体/L2 笼罩/L3 表意，已落 L0/L1）**：**L0 本体层** = `fx/unitBodyFx.js`，onBeforeCompile 挂单位 `_body` 材质（**每单位独立实例，无族单例复位负担**——与 charBurn 的房间道具族材质相反），单 program 多 uniform（uBurn 首发），BattleStage 建视图时挂 + `compileAsync` 入场预热（编译卡顿 715ms 教训）；本体亮度**压 bloom 阈下**（发光是 L1 的活，防糊白）。**L1 贴体层** = `fx/bodyFlames.js` 贴体叠火（orbs 同款 sprite 火贴体版）：挂 **billboard 子组**（贴体铁律：挂 unit 根的件不随立绘转身）、HDR 乘算过 1.45 阈真发光、`setLevel` 单标量总控、`unit.addTick` 抖焰（orbs 同惯例）。状态入口唯一 = AuraHost + `recipes.js` 的 `UNIT_AURA_THEMES`（diff 驱动铁律不动）；**存续 aura 的强度追层数走 `def.update`**（AuraHost `set()` 只管增删，同步侧逐个调）——burn 三通道（L0 本体余烬 + L1 叠火 + 余烬粒子）由 level 一个标量合控。aura 补间两条纪律：`overwrite:'auto'`（exit→0 与 reenter/update 撞车不拽回）+ `onInterrupt` 同落地（被顶补间不吞 Promise）。L2 笼罩层（凝滞等需下层 framebuffer 的特效：缺省同源重算零 RT）与 L3 表意层待 Phase 2。
