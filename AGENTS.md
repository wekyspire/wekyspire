...
- **`run/`** — run 层状态机：`runFlow.js`（createRun/enterBattle/finishBattle/advanceFloor…）、`runDriver.js`（headless 整局 SDK，自动应答结算期输入、自动开包与种子包）、rewards / ascension / prep、rooms/（camp、event、slotMachine、training）。**Boss 通关奖励三件套**：删卡机会 + HP 回满 + **遗物掉落**（`BOSS_RELIC_RARITY` 按章爬坡：章1 C/B、章2 B、章3 B/A、章4 A；走 draft.js 统一抽选 SDK，特写由 Shell 拥有集差分自动兜——2026-09-13 实装：设计稿早列了这个来源但一直没人写，导致遗物正常局一局只见 0-2 件）。
  ...
  - **跳过进阶**：不选灵脉，改记 1 点隐藏体修等级（`player.bodyLevel`，体修卡包门禁）+ **生命上限 +3**（第 7 轮裁决的跳过补偿，走 `gainMaxHp`），与灵脉路径同样享受定量恢复（10 点，2026-09 定案：全恢复使跳过/点火无脑化，回满血留给 Boss 通关）与魏启上限 +1，但不触发种子包——故事模式暗线（体修大成）。
