...
- `saves.js` — 存档（两模式隔离）；`settings.js`、`audio.js`、`tooltip.js`；`components/`（**剩余的 Vue 组件**：菜单层 StartScreen/GameMenu/EndPanel/MenuDialog·MenuPopup/ChangeLog、战斗 `BattleHud`（日志/回合数）、`TooltipOverlay`/`CardFacePreview`、顶层加载门 AssetLoadingScreen——全量美术**全部成功**预载前挡住开始界面；失败**卡住不放行**并给重试（用户定 2026-09-12：掐断下载不得带着缺图进游戏），进度条旁显示已下载/总大小、平均网速与 ETA——体积走 `assetManifest.preloadAllArt` 的 HEAD 探测（无 `Content-Length` 时退化为按张数估算），`onStats` 回调给原始量）、`overlay/`（cutscene 播放器 `cutscenePlayer.js` + 内容层 `CutsceneOverlay.vue` + **切幕层** `sceneWipe.js`/`SceneWipeOverlay.vue` + 剧本 `scripts.js`/事件图 `eventArt.js`）。**阶段操作面板已不在 Vue 里**（迁入 `stage/panels/`，见 Stage 段）。
